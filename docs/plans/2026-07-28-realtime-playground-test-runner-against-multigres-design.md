...
- CI/automated E2E (still a possible follow-up; the headless `realtime-check.ts` tool
  ...
  scope here).
- Any changes to the Playground repo. Everything needed is either already generic and
  upstream (see "Fixture setup" below) or is Multigres-specific glue that belongs in
  this repo.
